...
if __name__ == "__main__":
    ...
    parser.add_argument("--modelo", type=str, default="configuracion/yolov3.cfg",
                        help="ruta al archivo de definición del modelo ")
    parser.add_argument("--ruta_pesos", type=str,
                        default="pesos/checkpoint.pth", help="ruta al archivo de pesos")
    parser.add_argument("--ruta_nombre_clases", type=str,
                        default="datos/clases.names", help="ruta al archivo de etiqueta de clase")
    parser.add_argument("--ruta_imagenes", type=str,
                        default="imagenes/", help="ruta de la carpeta de imagenes")
    parser.add_argument("--umbral_confianza", type=float,
                        default=0.8, help="umbral de confianza del objeto")
    parser.add_argument("--webcam", type=int, default=1,
                        help="se utilizara una webcam 1 = Sí, 0 = no")
    parser.add_argument("--umbral_iou", type=float, default=0.4,
                        help="umbral de iou para supresión no máxima")
    parser.add_argument("--tamanio_cada_imagen", type=int,
                        default=416, help="tamaño de cada dimensión de la imagen")
    parser.add_argument("--directorio_video", type=str,
                        help="Directorio del video para la deteccion")
    parser.add_argument("--tipo_alarma", type=int, default=0,
                        help="forma de envio de alerta al detectar el objeto en cuestión, 0 para enviar por correo electronico, 1 para envio de msm de texto utilizando AWS con un tema en especifico, 2 para realizar envio por http ")
    parser.add_argument("--detectar_multiplesImagenes", type=int, default=0,
                        help="detectar cada vez que seingresauna nueva imagenal directorio de imagenes, para est se digita 1")
    ...
    model = Darknet(parametros.modelo,
                    parametros.tamanio_cada_imagen).to(device)
    ...
    if parametros.detectar_multiplesImagenes == 1:
        ...
        while(pregunta):
            ...
            try:
                ...
                out = cv2.VideoWriter('imagen'+ str(contador) +'.jpg', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 10, (1280, 960))
                ...
                for detection in detections:
                    if detection is not None:
                        ...
                        for x1, y1, x2, y2, conf, cls_conf, cls_pred in detection:
                            ...
                            print("se detectó: {} , con un acierto del: {} %".format(classes[int(cls_pred)],str("%.2f" % float(conf))))
                            ...
                            if parametros.tipo_alarma == 0:
                                ...
                                out2 = cv2.VideoWriter('imagenesSalida/1.jpg',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (1280,960))
                                out2.write(Convertir_BGR(RGBimg))
                                file = open("imagenesSalida/1.jpg", "rb")
                ...
                contador=contador-1
                print("no hay imagenes por el momento")
